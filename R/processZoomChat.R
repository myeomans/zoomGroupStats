...
#'
#' @description  Processing the chat in parallel with a zoom call
#' @param fname character the path to the local file where the chat file (txt) is saved.
#' @param sessionStartDateTime character the time that the actual session was launched. Format is YYYY-MM-DD HH:MM:SS
#' @param languageCode character language code. Default is "en".
#' @details This function parses the data from the chatfile that is downloaded from the Zoom website.
...
#' @return A data.frame containing a processed chat transcript with the following columns:
#'    \item{messageId}{an incremented numeric identifier for a message in the chat}
#'    \item{messageTime}{a timestamp for the message, based on the start of the Zoom session}
#'    \item{userName}{the name attached to the message}
#'    \item{message}{the text of the message}
#'    \item{messageLanguage}{the language code for the chat}
#' @examples
#'
#' \dontrun{
#' ch.out = processZoomChat(fname="~/Desktop/chat.txt", sessionStartDateTime="2020-04-01 17:56:34")
#' }      $MIKE$ - should probably default to read from internal character vector, with readLines option
#'
#'@export
processZoomChat = function(fname, sessionStartDateTime, languageCode="en") {
	...
	ch = utils::read.delim(fname, sep="\t",
	                stringsAsFactors=F, header=F,
	                col.names=c("messageIncrement", "userName", "message"), quote="")
	...
	# the parsing of the file is messed up when there are soft returns in someone's chat message
	...
	ch$userName = ifelse(ch$userName == "" | is.na(ch$userName),
	                     "UNIDENTIFIED",
	                     ch$userName)
